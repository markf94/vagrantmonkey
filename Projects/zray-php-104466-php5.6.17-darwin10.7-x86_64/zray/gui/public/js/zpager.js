var zPager = new Class({
	...
	draw: function() {
		...
		// create prev button
		if (maxItems > 1) {
			var prevBtn = new Element('li', {'class': 'prev glyphicons chevron-left'});
			prevBtn.inject(parent.list);
			
			var prevLink = new Element('a', {'href': 'javascript:void(0)', html: '<i></i>'});
		...
		// create next button
		if (maxItems > 1) {			
			var nextBtn = new Element('li', {'class': 'next glyphicons chevron-right'});
			nextBtn.inject(parent.list);			

			var nextLink = new Element('a', {'href': 'javascript:void(0)', html: '<i></i>'});
