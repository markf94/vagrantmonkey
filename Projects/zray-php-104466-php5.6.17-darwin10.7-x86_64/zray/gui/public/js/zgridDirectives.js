var zgridDirectives = new Class({
	...
	onLoadData: function (gridRowDetails, data)
	{
		if (data) {
			...
			var tabPane = new TabPane('tableDescContent_' + extension, {}, function() {
				...
				if (gridRowDetails.daemonTab == true) {
					return 1; // set first tab as active
				}
                return 0;
			}.bind(gridDataRow));
			...
			if (this.options.isAllowedToSaveDirectives) {
				...
				// apply degrading onchange hook to perform validation only for text fields. The rest have predefined values
				expandedContainer.getElements('select').addEvent('change', this.scheduleValidate.bind(this));
				expandedContainer.getElements('input[type=\'text\']').addEvent('input', this.scheduleValidate.bind(this));
			} else {
		...
		this.fireEvent('descriptionOpened', {row: gridRowDetails, data: data, tabPane: tabPane});
	},
	...
	validate: function (target, styleChangeTarget, name) {
		...
		if (typeof styleChangeTarget.spinner == 'undefined') {
			styleChangeTarget.spinner = new Spinner(styleChangeTarget.getParent());
			styleChangeTarget.spinner.addEvent('show', function(){
				styleChangeTarget.set('disabled', true);
				target.set('disabled', true);
				styleChangeTarget.spinner.resize();
			});
			styleChangeTarget.spinner.addEvent('hide', function(){
				styleChangeTarget.set('disabled', false);
	...
	onValidate: function(target, visualTarget, name, data) {
		...
		target.set('readonly', false);
		visualTarget.set('readonly', false);
		...
		if (data) {
			...
			if (validate.valid) {
				...
				if (! data.responseData.prerequsitesPluginValidation) {
                	
					var answer = confirm(_t('Directive ' + name + ' contradicts the required prerequisites for a deployed plugin. Do you wish to disable the (' + data.responseData.brokenPlugin + ') plugin first?'));
        			if (! answer){
        				this.deploymentValidate = false;
        				...
        				var request = new Request.WebAPI({url: baseUrl() + '/Api/disablePlugins', data: {"plugins": [data.responseData.brokenPluginId]}});
            	  		request.post();
        			}
				}
			} else {
				...
				var errorMessage = 'Directive ' + name + ' failed validation';
				if (data && data.responseData && data.responseData.validates &&  data.responseData.validates.length > 0) {
					errorMessages = [];
					data.responseData.validates.forEach(function(errData) {
						if (errData.name == name && errData.messageList && errData.messageList.length > 0) {
							errData.messageList.forEach(function(singleErrorData) {
								errorMessages.push(singleErrorData.error);
							});
						}
					});
	...
	loadData: function(data)
	{
		...
		if (data.daemonTab) {
			params.daemonTab = data.daemonTab;
		}
	...
	showMessages: function(messageList) {
		...
		for (var i = 0; i < messageList.length; i++) {
			var nodeMessageString = messageList[i].nodeName + messageList[i].message;
			// the message already was shown, don't show again #ZSRV-14591
			if(addedMessages.indexOf(nodeMessageString) != -1) {
				continue;
			}
			var nodeText = '';
			...
			addedMessages.push(nodeMessageString);
		}
		return messages;
	},
