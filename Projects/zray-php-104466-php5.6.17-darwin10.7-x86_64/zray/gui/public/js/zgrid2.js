var zGrid2 = new Class({
	...
	getOptions: function(){
		return {
			...
			menu:			null,
			totalContainer:	null,
			totalLabel:		_t('Showing'),
			totalEntriesLabel: _t('entries'),
			
			// header over the actions column (last col.)
			actionsColumnLabel: null,
			actionsColumnTooltip: null,
	...
	initialize: function(container, columnModel, options){
		...
		if (this.options.actionsColumnLabel) {
			actionsWidth = Math.max(this.options.actionsColumnLabel.length * 8, actionsWidth); // 8px is the avg letter width in the current font size  
		}

		// set minimum width to show label
		actionsWidth = Math.max(55, actionsWidth);
		...
		if (this.options.actionsColumnLabel) {
			actionsColumn.set('text', this.options.actionsColumnLabel);
		}
		
		if (this.options.actionsColumnTooltip) {
			actionsColumn.set('title', this.options.actionsColumnTooltip);
		}
		// add width of buttons
		if (this.buttons.length > 0) {
			actionsColumn.set('text', 'Actions');
			actionsColumn.set('title', 'Actions');
		}
	...
	moveRow: function(rowId, nearId, position) {
		if (this.container.getElement('#tableRow_' + rowId) && this.container.getElement('#tableRow_' + nearId)) {
			this.container.getElement('#tableRow_' + rowId).inject(this.container.getElement('#tableRow_' + nearId), position);
			if (this.options.description) {
				$('tableDescRow_' + rowId).inject(this.container.getElement('#tableRow_' + rowId), 'after');
			}
	...
	updateData: function(data, position) {
		...
		if (hash && this.options.rowExpand) {
			hash.each(function(item) {
				if (this.container.getElement('#tableRow_' + item) && ! this.container.getElement('#tableRow_' + item).hasClass('active')) {
	...
	setRow: function(rowData, type, nearId, rowExpand) {
		...
		if (this.container.getElement("#tableRow_" + rowId)) {
	...
	deleteRow: function(rowId) {
		...
		this.container.getElement('#tableRow_' + rowId).dispose();
	...
	expandRow: function(rowId, rowData) {
		...
		this.container.getElement("#tableRow_" + rowId).toggleClass("active");
		...
		if (this.container.getElement("#tableRow_" + rowId).hasClass('active')) {
	...
	getDescriptionParams: function(rowId) {
		return {id: rowId, rowId: 'descItemTr_' + rowId, data: this.getRowData(rowId), row: this.container.getElement("#tableRow_" + rowId)};
	},
	...
	isChecked: function(rowId) {
		var checkbox = this.container.getElement('#tableRow_' + rowId).getElement('input[type="checkbox"]');
		...
		if ($(this.options.totalContainer)) {
			...
			if (totalElement) {
				...
				if (typeof pager !== 'undefined') {
					...
					if (this.totalRows == 0) {
						totalElement.set('html', this.options.totalLabel + ' ' + displayNow + ' to ' + itemsDisplayed + ' of ' + this.totalRows + ' ' + this.options.totalEntriesLabel);
					} else if (itemsDisplayed == this.totalRows && this.page == 1) {
						totalElement.set('html', this.options.totalLabel + ' ' + itemsDisplayed + ' of ' + this.totalRows + ' ' + this.options.totalEntriesLabel);
					} else {						
						totalElement.set('html', this.options.totalLabel + ' ' + (displayNow + 1) + ' to ' + itemsDisplayed + ' of ' + this.totalRows + ' ' + this.options.totalEntriesLabel);
					}
				} else {
					totalElement.set('html', this.options.totalLabel + ' ' + this.totalRows + ' of ' + this.totalRows + ' ' + this.options.totalEntriesLabel);
				}
			} else {
	...
	setNewRow: function(rowData, type, nearId, rowExpand) {
		...
		this.columnsOrder.each(function(rowExpand, columnIndex) {
			...
			if (rowData[columnIndex] != undefined && this.columnModel[columnIndex]) {
				...
				columnData = this.breakData(this.columnModel[columnIndex].break, columnData);
				if (typeof columnData == 'object') {
					rowColumn.grab(columnData); 
				} else {
			...
			if (rowExpand) {
				rowColumn.addEvent('click', function(e) {
					if (e.target.hasClass('zgrid-clickable') || e.target.hasClass('zgrid-status-ok') || e.target.hasClass('zgrid-status-progress') || e.target.hasClass('zgrid-status-error') || e.target.hasClass('zgrid-status-disabled') || e.target.nodeName == "TD" || (e.target.nodeName == "DIV" && e.target.hasClass('zgrid_ellipsis'))) { // catch only click on td
						this.expandRow(rowId, rowData);
					}
					this.fireEvent('rowClicked', {'rowId': rowId});	
				}.bind(this));
			} else {
				rowColumn.addEvent('click', function(e) {
					this.fireEvent('rowClicked', {'rowId': rowId});
		...
		if (rowExpand) {
			var descIndicator = new Element('span', {'id': 'descIndicator_' + rowId, 'class': 'tableDescClosed tableDescIndicator glyphicons chevron-right'});
			...
			var iElement = new Element('i');
			iElement.inject(descIndicator);
		...
		tableRow.addEvent('mouseleave', function() {
			if (this.container && this.container.getElement('#tableRow_' + rowId) && !this.container.getElement('#tableRow_' + rowId).hasClass('active')) {
		...
		this.buttons.each(function(item) {
			...
			if ((! item.display) || (item.display(rowData))) {
				...
				button.addEvent('click', function(event) {
					this.fireEvent('buttonClick', {'rowId': rowId, 'type': dataIndex, 'data': rowData, 'btn': event.target});				
				}.bind(this));
			}
		}.bind(this));
	...
	updateRow: function(rowData) {
		var rowId = this.getRowId(rowData);
		var rowElements = this.container.getElement('#tableRow_' + rowId).getElements('td');
		...
		Object.each(rowData, function(column, key) {
			...
			if (this.columnModel[key]) {
				...
				columnData = this.breakData(this.columnModel[key].ellipsis, columnData);
	...
	createTableRow: function(parent, styles, type, nearId) {
		...
		if (type) {
			...
			if (nearId && this.container.getElement('#tableRow_' + nearId)) {
				if (type == 'before') {
					tableRow.inject(this.container.getElement('#tableRow_' + nearId), type);
				} else if (type == 'after' && $('tableDescRow_' + nearId)) {
					tableRow.inject($('tableDescRow_' + nearId), type);
				} else if (type == 'after' && this.container.getElement('#tableRow_' + nearId)) {
					tableRow.inject(this.container.getElement('#tableRow_' + nearId), type);
				}
			} else {
	...
	breakData: function(breakdata, data) {
		if (breakdata !== undefined && breakdata === false) {
			return data;
		}
		
		data = '' + data; // make sure it's a string
		return '<div class="zgrid_break" title="' + data.stripTags() + '">' + data + '</div>';
	},
