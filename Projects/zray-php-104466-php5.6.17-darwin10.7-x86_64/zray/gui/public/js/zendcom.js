...
var ZendLicenseRetriever = new Class({
	...
	'_sendUiMessage': function(edition, daysToExpired) {
		if (daysToExpired != undefined && daysToExpired > 0) {
			$('licenseFrame').set('src', this.protocol + '//www.zend.com/products/server/license/product?retUrl=' + encodeURIComponent(location.href) + '&edition=' + edition + '&version=' + this.zsVersion + '&daysLeft=' + daysToExpired + this.extraParams);
		} else {			
			$('licenseFrame').set('src', this.protocol + '//www.zend.com/products/server/license/product?retUrl=' + encodeURIComponent(location.href) + '&edition=' + edition + '&version=' + this.zsVersion + this.extraParams);
		}
	},
	/**
	 * @private
