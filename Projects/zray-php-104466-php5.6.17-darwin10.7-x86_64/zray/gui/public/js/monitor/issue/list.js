var monitorIssueList = new Class({
	...
	deleteFiltered: function() {
		...
		var params = {"filters":filterWidget.selectedFilters};
  		...
  		$$('#delete-filtered .count').set('html', '');
	...
	checkItems: function() {
		...
		if (this.options.pager.totalItems > 0) {
			...
			if (this.totalCount > 0) {
				$$('#delete-filtered .count').set('html', ' ('+this.options.pager.totalItems+')');
			}
		} else {
			$('delete-filtered').set('disabled', true);
		}
	...
	_initGrid: function() {
		...
		this.loadRequest = new Request.WebAPI({
			...
			onFailure: function(response) {
				...
				var messageText = response && response.errorData && response.errorData.errorMessage ? response.errorData.errorMessage : 'Error occured';
				document.fireEvent('toastAlert', {'message': messageText});
				zgrid2.postLoad();
				zgrid2.setData([], 0);
    	...
    	zgrid2.addEvent('rowChecked',function(params) {
            rowsChecked=$$('tbody tr[id^="tableRow"] .zgrid-checkbox:checked').length;
            if (params.checked) {
    			if (this.options.allowedDelete && $('delete')) {
                    if (rowsChecked >= 1) {
                        $('delete').set('disabled', false);
                    }
                    $$('#delete .count').set('html', ' ('+rowsChecked+')');
    			}
    		} else {
                if($('delete')) {
                    if (rowsChecked == 0) {
	...
	whatHappened: function(value, data) {
			...
			requestToText = 'Request to ';
			
			if (data.generalDetails.url) {
				requestUrl = data.generalDetails.url; 
			}
			
			if (data.routeDetails) {
				requestToText = 'Request with route: ';
				requestUrl = data.routeDetails;
			}
			...
			switch (eventType) {
				...
				case 3: // ZM_TYPE_REQUEST_RELATIVE_SLOW_EXEC, ZM_TYPE_REQUEST_SLOW_EXEC
	  				if (value.relExecTime > 0) {// REL
	  	  				return monitorIssueList.prototype.clean(requestToText + requestUrl + ' took ' + Math.round(value.relExecTime) + '% more than normal');
	  				}else { // ABS
	  	  				return monitorIssueList.prototype.clean(requestToText + requestUrl + ' took ' + value.execTime + 'ms');
	  				}  							
				case 4: // ZM_TYPE_REQUEST_RELATIVE_LARGE_MEM_USAGE, ZM_TYPE_REQUEST_LARGE_MEM_USAGE
	  				if (value.relMemUsage > 0) {// REL
	  	  				return monitorIssueList.prototype.clean(requestToText + requestUrl + ' consumed ' + Math.round(value.relMemUsage) + '% more than normal');
	  				}else { // ABS
	  	  				return monitorIssueList.prototype.clean(requestToText + requestUrl + ' consumed ' + value.memUsage + 'KB');
	  				}  	
				case 5: // ZM_TYPE_REQUEST_RELATIVE_LARGE_OUT_SIZE
					return monitorIssueList.prototype.clean(requestToText + requestUrl + ' generated output of ' + Math.round(value.relOutputSize) + '% more than normal');
