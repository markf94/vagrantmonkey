...
var FloatingTips = new Class({
	...
	_create: function(elem) {
		...
		var elementIsFixed = function(elem) {
			var isFixed = false;
			while (elem && elem.style) {
				if (elem.getStyle && elem.getStyle('position')=='fixed') {
					isFixed = true;
					break;
				}
		...
		if (elementIsFixed(elem)) {
			trgC.top -= document.body.scrollTop;
			tip.setStyles({position:'fixed'});
		}
		...
		var htmlWidth = $$('html').getWidth()[0];
		if((tip.getLeft() + tip.getWidth() - htmlWidth) > 0){
			tip.setStyle('left',tip.getLeft() - (tip.getLeft() + tip.getWidth() - htmlWidth) - 10 );
		}
