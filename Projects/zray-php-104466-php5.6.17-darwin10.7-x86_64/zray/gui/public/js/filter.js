var filter = new Class({
	...
	showFilterBar: function (parent, type, fragment) {
		...
		var aShowFilter = new Element('a', { 	'id': 'show_filter_details',
			'html'  : _t('Show Filters') + '<i></i>',
			'href' : 'javascript:void(0)',
			'class' : 'showFilterDetails glyphicons white chevron-right hidden',
			events: {
				click: function(){
		...
		var aHideFilter = new Element('a', { 	'id': 'hide_filter_details',
			'html'  : _t('Hide Filters') + '<i></i>',
			'href' : 'javascript:void(0)',
			'class' : 'hideFilterDetails glyphicons white chevron-right',
			events: {
				click: function(){
					this.toggleFilterDetails(false);
				}.bind(this)
		...
		var divVisibleFiltersButtons = new Element('div', {'class': 'visibleFiltersButtons'});
		divVisibleFiltersButtons.inject(divFilterBar);
		...
		var deleteButton = new Element ('div', {'id': 'clear_filter_btn', 'title' : _t('Clear filters'), 'html' : '<i></i>', 'class': 'glyphicons bin'});
		...
		deleteButton.inject(divVisibleFiltersButtons);
		...
		if (this.externalFilters.length > 0) {
			...
			var inputWrapper = new Element('span', {'class': 'filter-input-wrapper'});
			timeInput.inject(inputWrapper);
			timeImage.inject(inputWrapper);
			
			timeLabel.inject(timeSpan);
			inputWrapper.inject(timeSpan);
			timeSpan.inject(timeRangeFilter);
	...
	showSelectedFilterRow: function (parent) {
		...
		contentTd = new Element('td', {'class': 'selectedFiltersContainer'});
		contentTd.inject(divSelectedFiltersBox);
		actionsTd = new Element('td', {'class': 'selectedFiltersBarActions'});
		actionsTd.inject(divSelectedFiltersBox);
		...
		saveasButton.inject(actionsTd);
		
		var saveButton = new Element ('button', {'id': 'save_filter_btn', 'title' : _t('Save filter'), 'html' : _t('Save')});
		saveButton.addEvent('click', function(){this.saveFilterAction();}.bind(this));
		saveButton.inject(actionsTd);
	},
	...
	saveFilterAction: function(isNewFilter) {
	...
	selectSearchFilter: function(query, filter, filterId, filterName) {
		...
		closeBtn.inject(spanFilterName, 'top');
		...
		spanFilterName.addEvent('click', function(filter,item) {
			...
			// the remove click was on the text itself bug #ZSRV-15209
			if (item.target.id == "search") {				
				item.target.dispose();
			} else { //  on on on the button of 'x' - remove the whole span
				item.target.parentElement.dispose();				
			}
