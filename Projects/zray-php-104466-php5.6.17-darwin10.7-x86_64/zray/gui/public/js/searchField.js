var SearchField = new Class({
	...
	_render: function() {
		...
		$$('.' + className).each(function(item) {
			...
			var selectSearch = new Element('a', {'class': 'selectSearch glyphicons search', 'id': 'freeText', 'title': 'search', 'href': 'javascript:void(0);'});
			selectSearch.set('html', '<i></i>');
			selectSearch.addEvent('click', function() {
				this.fireEvent('searchClicked', {'value': searchInput.get('value'), 'element': searchInput});
			}.bind(this));

			var clearSearch = new Element('a', {'class': 'clearSearch glyphicons remove_2 hidden', 'title': 'clear', 'href': 'javascript:void(0);'});
			clearSearch.set('html', '<i></i>');
